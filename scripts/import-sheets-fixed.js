/**
 * Google Sheets Veri Import Script - DÜZELTİLMİS
...
// Google Sheets'ten okunan TÜM veriler (sadece dolu satırlar)
const SHEETS_DATA = [
  {
    id: '2720',
    tarih: '03.02.2026',
...
async function importData() {
  ...
  // 1. Benzersiz tekneleri çıkar ve kaydet
  ...
  for (const tekne of tekneler) {
    ...
    if (!existing) {
      await prisma.tekne.create({ data: tekne });
      createdTekneler++;
      console.log(`  ➕ ${tekne.ad}`);
    }
  }
  console.log(`\n✅ ${createdTekneler} yeni tekne kaydedildi\n`);
  ...
  for (const item of SHEETS_DATA) {
    try {
      ...
      if (existing) {
        ...
        await prisma.servis.create({ data: servisData });
        createdServisler++;
