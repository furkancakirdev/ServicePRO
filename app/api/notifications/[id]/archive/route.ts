  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await writeAuditEvent(tx, {
        ...
        islemTuru: JOB_AUDIT_EVENTS.NOTIFICATION_ARCHIVED,
        entityTipi: 'Notification',
        entityId: existing.id,
        detay: `Bildirim arsivlendi: ${existing.id}`,
      });
    ...
    console.error('POST /api/notifications/[id]/archive error:', error);
    return NextResponse.json({ error: 'Bildirim arsivlenemedi' }, { status: 500 });
  }
}
